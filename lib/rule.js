...
(function () {
   ...
   * @static;
   */
  Rule.type = function (type) {
    var Klass = this,
        proto = Klass.types[type];
    if (!proto)
      return null;
    if (proto.create && _.isFunction(proto.create))
      proto.api = proto.create(Klass);
...
//#JSCOVERAGE_IF
  ...
  Rule.prototype.dump = function (i) {
    ...
    console.log(
      indent(
        i, (this.resolveNS() || '.') + ' /' + this.matcher.asString() + '/'));
...
//#JSCOVERAGE_ENDIF
   ...
   * @see {@link RUle#tokenNamespace}
   */
  Rule.prototype._resolveNamespaces = function () {
    if (this.namespace) {
      this.matcher.name(this.namespace);
    }
    this.forEach(function (rule) {
  ...
  Rule.prototype._resolveMatcher = function () {
    ...
    this.forEach(function (rule) {
      ...
      if (!rule.getOption('exclusive') &&
          rule.matcher && rule.matcher.hasSpecs()) {
        var subMatcher = rule.matcher.clone();
        subMatcher.name(rule.tokenNamespace());
        this.matcher.add(subMatcher);
      }
    }.bind(this));
  };
  /**
