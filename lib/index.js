...
(function () {
  ...
  Lex.prototype.setState = function (state) {
    var res = this.resolveSiblingNS(state);
    if (!res)
      return this.handleError("invalid state " + state);
    this._stateStack = [res];
    return this;
  };
  /**
  ...
  /**
   * resolves a namespace to a {@link Lex~State} object. Assumes it could
   * possible be a child of the current state and tries to do a relative
   * resolution first.
   *
  ...
  Lex.prototype.resolveSiblingNS = function (ns) {
    if (this.curState && this.curState.parent) {
      var curNs = this.curState.parent + this.curState.sep + ns;
      if (this._states[curNs])
        return this._states[curNs];
    }
    if (this._states[ns])
  ...
  Lex.prototype._scan = function () {
    ...
    if (!this.reject) {
      ...
      if (tokens.length === 0) {
         ...
         * up above is enough to continue to the next state.
         */
